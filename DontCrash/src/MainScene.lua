...
-- 2015/03/03
------------------------------------------------
----runtime/win32/PrebuiltRuntimeLua.exe
----名字改为：cocos2d-x-3.2 --> cocos2d-x
--使用require引入其他模块
...
function MainGameScene:StartGame(layer)
	self.play = true;
...
function MainGameScene:setUI(layer)
    self.score = cc.Label:createWithBMFont("fonts/font_greenNum.fnt","0")
...
function MainGameScene:mainLogic(layer)
	...
	--test
	print("轨道长宽", self.track_size.width, self.track_size.height)
	print("bottomy and topy", self.car_bottomy, self.car_topy)
	...
	print("初始位置:", car_red:getPositionX()+car_red:getContentSize().width, self.track_size.height*0.063)
...
function MainGameScene:runCar(car)
    ...
    --创建第一个动作
	print(111, self.track_size.height, self.track_size.height*0.445)
...
function MainGameScene:setButton(imageName,layer)
    ...
    --判断游戏是否开始的变量
    self.play = false
    print("setButton: self.play", self.play)
end
...
function MainGameScene:resetCar()
    ...
    self.car_red:setRotation(0)

    --黄色小车重新设置位置和角度
    self.car_yello:setPosition(self.car_red:getPositionX()+self.car_red:getContentSize().width,self.track_size.height*0.063)
    self.car_yello:setRotation(0)
end
--end
